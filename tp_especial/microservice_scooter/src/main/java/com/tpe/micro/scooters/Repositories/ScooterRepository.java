...
public interface ScooterRepository extends JpaRepository<Scooter, Long> {
    ...
    @Query("SELECT s FROM Scooter s WHERE s.longitude BETWEEN :minLongitude AND :maxLongitude AND s.latitude BETWEEN :minLatitude AND :maxLatitude AND s.available = true")
    List<Scooter> findNearby(Double minLongitude, Double maxLongitude, Double minLatitude, Double maxLatitude);

    @Query("SELECT s FROM Scooter s WHERE s.available = true ORDER BY (POWER(s.latitude - :latitude, 2) + POWER(s.longitude - :longitude, 2)) ASC LIMIT 1")
    Scooter findAScooterNear(Double latitude, Double longitude);
