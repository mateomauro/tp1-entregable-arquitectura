...
public interface MaintenanceRepository extends JpaRepository<Maintenance, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE Maintenance m SET m.id_skateboard = :id_skateboard, m.repair = :repair WHERE m.id_maintenance = :id")
    public void updateOne(@Param("id") long id, @Param("id_skateboard") long id_skateboard, @Param("repair") Boolean repair);

    @Transactional
    @Query("SELECT new org.example.microservicemaintenance.DTOs.MaintenanceScooterDTO(m.id_skateboard,m.repair) FROM Maintenance m WHERE m.repair = false")
